...
export default function VictimWelfareDashboard() {
  const { id } = useParams();

  return (
    <div className="welfare-page" style={{ padding: '1rem' }}>
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.75rem', fontWeight: 'bold', color: '#111827' }}>
          Welfare Support Dashboard {id && <span>(ID: {id})</span>}
        </h1>
      ...
      <div style={{ backgroundColor: '#ffffff', padding: '3rem 2rem', borderRadius: '8px', border: '1px dashed #d1d5db', textAlign: 'center', color: '#6b7280' }}>
        <p style={{ fontSize: '1.1rem' }}>Welcome to the Victim Welfare Module. Active requests will appear here.</p>
      </div>
    </div>
  );
}
